...
class Imap extends Base
{
	 ...
	 * @return Eden\Mail\Imap
	 */
	private function debug($string, $stdout = true)
	{
		if ($this->debugging) {
	...
	private function getEmailFormat($email, $uniqueId = null, array $flags = array())
	{
		...
		$recipientsTo = $recipientsCc = $recipientsBcc = $sender = $extraTo = array();
		...
		// check from and to email are the same
		if (isset($headers1->from[0], $headers1->to[0]) AND $headers1->from[0] == $headers1->to[0]) {
			if (isset($headers2['to'])) {
				// Parse To emails from Headers 2.
				$arrEmails = explode(",", $headers2['to']);
				foreach($arrEmails as $no => $toEmail) {
					$resEmail = $this->_parseRecepient(imap_rfc822_parse_headers_decode($toEmail));
					if ($sender['email'] == $resEmail['email']) {
						$extraTo[] = $resEmail;
					} else {
						$recipientsTo[] = $resEmail;
					}
				}
			}
		} else {
			//set the to
			$recipientsTo = $this->_parseRecepient($headers1->to);
		}
		
		//set the cc
		$recipientsCc = $this->_parseRecepient($headers1->cc);
		
		//set the bcc
		$recipientsBcc = $this->_parseRecepient($headers1->bcc);
		
		if (!empty($extraTo)) {
			foreach($extraTo as $cc)
				$recipientsCc[] = $cc;
		}
	...
	private function _parseRecepient($objRecps = null)
	{
		if ($objRecps == null)
			return null;
		...
		$recipients = array();
		if (isset($objRecps)) {
			foreach ($objRecps as $recp) {
				if (!isset($recp->mailbox, $recp->host)) {
					continue;
				}
				$recipient = array('name'=>null);
				if (isset($recp->personal)) {
					$recipient['name'] = $recp->personal;
					//if the name is iso or utf encoded
				...
				$recipient['email'] = $recp->mailbox . '@' . $recp->host;
